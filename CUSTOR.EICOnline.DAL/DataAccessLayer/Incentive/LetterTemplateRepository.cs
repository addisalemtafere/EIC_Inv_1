...
namespace CUSTOR.EICOnline.DAL.DataAccessLayer
{
    public class LetterTemplateRepository : EFRepository<ApplicationDbContext, LetterTemplate>
    {
        ...
        public Task<List<LetterTemplateDTO>> GetLetterTemplates(string lang, int page = 0, int pageSize = 15)
        {
            string FieldName = StaticDataHelper.GetFieldName(lang);
            string query1 =$@"(select (Select {FieldName} from Lookup Where LookUpTypeId='707' AND Lookup.LookupId=LetterTemplate.LetterType) as LetterType,LetterTemplateId from LetterTemplate)";
