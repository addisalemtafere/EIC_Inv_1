...
export class IncentiveRequestService {
  ...
  getIncentiveRequestByServiceApplicationIdForLetter(id, lang): Observable<IncentiveRequestModel[]> {
    return this.httpClient.get<IncentiveRequestModel[]>(this.config.urls.url('incentiveRequestByServiceAppIdForLetter', id, lang)).pipe(
      map(incentiveRequestdata => {
        this.incentiveRequestList = incentiveRequestdata;
        return this.incentiveRequestList;
      }),
