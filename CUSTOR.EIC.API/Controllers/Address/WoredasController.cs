...
namespace CUSTOR.EICOnline.API.Controllers.Address
{
  ...
  public class WoredasController
  {
    ...
    [HttpGet]
    [Route("api/woredaByParent/{id}")]
    public async Task<IEnumerable<Woreda>> GetWoredasByParent(string id,int page = -1, int pageSize = 10)
    {
      return await _WoredaRepo.GetWoredasByParent(id,page, pageSize);
    }
