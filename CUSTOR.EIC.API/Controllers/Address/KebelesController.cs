...
namespace CUSTOR.EICOnline.API.Controllers.Address
{
  ...
  public class KebelesController
  {
    ...
    [HttpGet]
    [Route("api/kebeles/ByParent/{id}")]
    public async Task<IEnumerable<Kebele>> GetKebelesByParent(string id, int page = -1, int pageSize = 10)
    {
      return await _KebebeleRepo.GetKebelesByParent(id, page, pageSize);
    }
