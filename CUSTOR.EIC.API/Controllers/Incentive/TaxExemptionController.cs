...
namespace CUSTOR.EICOnline.API.Controllers
{
  ...
  public class TaxExemptionController : Controller
  {
    ...
    [HttpGet]
    [Route("api/taxexemptions/{id:int}/{lang}")]
    public async Task<IEnumerable<IncentiveTaxExemptionRequest>> GetTaxExemptions(string lang, int id, int page = -1,
      int pageSize = 10)
    {
      return await _TaxExemptionRepoo.GetTaxExemptions(lang, id, page, pageSize);
    }
