...
//    }
    ...
    public async Task<IEnumerable<SubSectorDTO>> GetSubSector(string lang, int page = -1, int pageSize = 10)
    {
      return await _SubSectorRepo.GetSubSectors(lang, page, pageSize);
    }

    [HttpGet("api/subsector/ByParent/{id}")]
    public async Task<IEnumerable<SubSector>> GetSubSectorByParent(int id, int page = -1, int pageSize = 10)
    {
      return await _SubSectorRepo.GetSubSectorsByParent(id, page, pageSize);
    }
