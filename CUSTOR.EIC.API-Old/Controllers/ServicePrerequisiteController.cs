...
namespace EIC.Investment.API.Controllers.Controllers
{
  ...
  public class ServicePrerequisiteController : Controller
  {
    ...
    [HttpGet]
    [Route("api/serviceprerequisite/ByParent/{id}")]
    public async Task<List<ServicePrerequisite>> GetServicePrerequisitesByParent(int id)
    {
      return await ServicePrerequisiteRepo.GetAllServicePrerequisitesByParent(id);
    }
